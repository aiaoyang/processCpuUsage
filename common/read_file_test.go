...
func isPidRunning(pids ...string) (pid []string, hasDeadPid bool) {
	deadPids := []string{}
	hasDeadPid = false
	// 如果pids 为空，则统一认为是死进程
	if len(pids) == 0 {
		hasDeadPid = true
		return pids, hasDeadPid
	}
...
func getAllProcess() map[string][]byte {
	res := make(map[string][]byte)
	fileInfoList, err := ioutil.ReadDir("/proc/")
	if err != nil {
		log.Fatal(err)
	}
	for _, pid := range fileInfoList {
		ok, _ := regexp.Match("[1-9]+", []byte(pid.Name()))
		if err != nil {
			log.Fatal(err)
		}
		if !ok {
		...
		cmdLine, err := ioutil.ReadFile("/proc/" + pid.Name() + "/cmdline")
		...
		// println(string(cmdLine))
		res[pid.Name()] = cmdLine
